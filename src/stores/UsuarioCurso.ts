...
export const useUsuarioCursoStore = defineStore("usuarioCurso", () => {
  ...
  async function fetchAllInscripciones() {
    try {
      const response = await fetch("/api/UsuarioCurso");
  ...
  async function fetchInscripcionesByUsuarioId(idUsuario: number) {
    try {
      const response = await fetch(`/api/UsuarioCurso/usuario/${idUsuario}`);
  ...
  async function fetchInscripcionesByCursoId(idCurso: number) {
    try {
      const response = await fetch(`/api/UsuarioCurso/curso/${idCurso}`);
  ...
  async function createInscripcion(nuevaInscripcion: UsuarioCursoDTO) {
    try {
      console.log("Enviando POST a /api/UsuarioCurso:", nuevaInscripcion);
      ...
      try {
        ...
        const response2 = await fetch("/api/UsuarioCurso", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const response3 = await fetch("/api/UsuarioCurso", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function deleteInscripcion(idUsuario: number, idCurso: number) {
    try {
      ...
      try {
        ...
        const urlDelete = `/api/UsuarioCurso?idUsuario=${idUsuario}&idCurso=${idCurso}`;
